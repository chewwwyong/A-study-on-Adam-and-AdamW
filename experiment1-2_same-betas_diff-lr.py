...
if __name__ == '__main__':
    ...
    for betas in tqdm(betas_list):
        ...
        for lr in tqdm(lr_list):
            ...
            classification_report1_DF.to_csv(f'runs/experiment1-2/adam_with_bias_correction-{lr:.0e}-{betas}-classification_report.csv', index=True)
            ...
            trainLossDF.to_csv(f'runs/experiment1-2/adam_with_and_without_bias_correction-{lr:.0e}-{betas}-training_loss.csv', index=False)
            ...
            trainAccDF.to_csv(f'runs/experiment1-2/adam_with_and_without_bias_correction-{lr:.0e}-{betas}-training_accuracy.csv', index=False)
            ...
            validationLossDF.to_csv(f'runs/experiment1-2/adam_with_and_without_bias_correction-{lr:.0e}-{betas}-validation_loss.csv', index=False)
            ...
            validationAccDF.to_csv(f'runs/experiment1-2/adam_with_and_without_bias_correction-{lr:.0e}-{betas}-validation_accuracy.csv', index=False)

            # Plot confusion_matrix1 figure 
            plt.clf()
            fig, ax = plot_confusion_matrix(conf_mat=confusion_matrix1)
            plt.title("Confusion Matrix")
            plt.savefig(f"assets/experiment1-2/adam_with_bias_correction-{lr:.0e}-{betas}-confusion_matrix.png")

            # Plot confusion_matrix2 figure 
            plt.clf()
            fig, ax = plot_confusion_matrix(conf_mat=confusion_matrix2)
            plt.title("Confusion Matrix")
            plt.savefig(f"assets/experiment1-2/adam_without_bias_correction-{lr:.0e}-{betas}-confusion_matrix.png")


        ############  Plot Adam w/ and w/o BiasCorr Training Loss & Accuracy  ############
        # Plot Adam with and without bias correction training loss with different lr
        plt.clf()
        for i, train_loss1 in enumerate(train_loss1s):
            plt.plot(train_loss1, label=f'Adam_lr={lr_list[i]}')
        for i, train_loss2 in enumerate(train_loss2s):
            plt.plot(train_loss2, label=f'Adam_w/o_BiasCorr_lr={lr_list[i]}')
        plt.legend()
        plt.title(f'Adam w/ & w/o Bias Correction Training Loss with Betas={betas} & Different lr')
        plt.xlabel('Epoch')
        plt.xticks(np.arange(0, num_epochs+1, 10))
        plt.ylabel('Loss')
        #plt.yticks(np.arange(0, 0.5, 0.1))
        plt.savefig(f'assets/experiment1-2/adam_with_and_without_bias_correction-{betas}-training_loss.png')

        # Plot Adam with and without bias correction training accuracy with different lr
        plt.clf()
        for i, train_acc1 in enumerate(train_acc1s):
            plt.plot(train_acc1, label=f'Adam_lr={lr_list[i]}')
        for i, train_acc2 in enumerate(train_acc2s):
            plt.plot(train_acc2, label=f'Adam_w/o_BiasCorr_lr={lr_list[i]}')
        plt.legend()
        plt.title(f'Adam w/ & w/o Bias Correction Training Accuracy with Betas={betas} & Different lr')
        plt.xlabel('Epoch')
        plt.xticks(np.arange(0, num_epochs+1, 10))
        plt.ylabel('Accuracy')
        #plt.yticks(np.arange(98, 100, 0.25))
        plt.savefig(f'assets/experiment1-2/adam_with_and_without_bias_correction-{betas}-training_accuracy.png')
        
        ############  Plot Adam w/ and w/o BiasCorr Validation Loss & Accuracy  ############
        # Plot Adam with and without bias correction validation loss with different lr
        plt.clf()
        for i, val_loss1 in enumerate(val_loss1s):
            plt.plot(val_loss1, label=f'Adam_lr={lr_list[i]}')
        for i, val_loss2 in enumerate(val_loss2s):
            plt.plot(val_loss2, label=f'Adam_w/o_BiasCorr_lr={lr_list[i]}')
        plt.legend()
        plt.title(f'Adam w/ & w/o Bias Correction Validation Loss with Betas={betas} & Different lr')
        plt.xlabel('Epoch')
        plt.xticks(np.arange(0, num_epochs+1, 10))
        plt.ylabel('Loss')
        plt.savefig(f'assets/experiment1-2/adam_with_and_without_bias_correction-{betas}-validation_loss.png')
        
        # Plot Adam with and without bias correction validation accuracy with different lr
        plt.clf()
        for i, val_acc1 in enumerate(val_acc1s):
            plt.plot(val_acc1, label=f'Adam_lr={lr_list[i]}')
        for i, val_acc2 in enumerate(val_acc2s):
            plt.plot(val_acc2, label=f'Adam_w/o_BiasCorr_lr={lr_list[i]}')
        plt.legend()
        plt.title(f'Adam w/ & w/o Bias Correction Validation Accuracy with Betas={betas} & Different lr')
        plt.xlabel('Epoch')
        plt.xticks(np.arange(0, num_epochs+1, 10))
        plt.ylabel('Accuracy')
        plt.savefig(f'assets/experiment1-2/adam_with_and_without_bias_correction-{betas}-validation_accuracy.png')